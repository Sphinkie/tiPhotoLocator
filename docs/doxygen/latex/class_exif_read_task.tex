...
\begin{DoxyCompactList}\small\item\em Constructeur. On enregistre le chemin et le nom du fichier JPG à lire. A noter que si on passe un nom de chemin, le process va traiter toutes les images du dossier. Cependant, on évite de le faire car, en termes de performances, ce n\textquotesingle{}est pas optimisé. \end{DoxyCompactList}\item 
...
\begin{DoxyCompactList}\small\item\em A appeler lors de la première utilisation. Mémorise quelques infos statiques. \end{DoxyCompactList}\end{DoxyCompactItemize}
...
\end{DoxyNote}
Exiftool json options Description of {\bfseries{JSON}} options for {\ttfamily Exif\+Tool}. 
\begin{DoxyCode}{0}
...
\DoxyCodeLine{\ \ -\/g\ to\ organize\ the\ output\ into\ objects\ by\ group,}
\DoxyCodeLine{\ \ or\ -\/G\ to\ add\ group\ names\ to\ each\ tag.}
...
Constructeur. On enregistre le chemin et le nom du fichier JPG à lire. A noter que si on passe un nom de chemin, le process va traiter toutes les images du dossier. Cependant, on évite de le faire car, en termes de performances, ce n\textquotesingle{}est pas optimisé. 
...
A appeler lors de la première utilisation. Mémorise quelques infos statiques. 
